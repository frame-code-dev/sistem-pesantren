...
<div class="p-4 mt-14">
	<section class="p-5 overflow-y-auto">
					...
					<ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
						...
						<li aria-current="page">
							<div class="flex items-center">
								...
								<span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Edit data pemasukan lainnya</span>
							</div>
						</li>
					</ol>
				</nav>
		...
		<div class="card bg-white p-5 mt-4 border rounded-md w-full relative">
			<form action="<?= base_url('dashboard/pemasukan-lainnya/update/' . $data['id']) ?>" method="POST" class="w-full mx-auto space-y-4" enctype="multipart/form-data">
				<div class="grid grid-cols-4 gap-3">
					<div class="col-span-2">
						...
						<select id="jenis" name="jenis" class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
							...
							<?php foreach ($data_jenis as $item) : ?>
								<option <?= $data['jenis_id'] == $item['id'] ? 'selected' : '' ?> value="<?= $item["id"] ?>"><?= $item["nama"] ?></option>
							<?php endforeach; ?>
						</select>
					...
					<div class="col-span-2 <?php if ($data['jenis_id'] == 7) : ?> hidden <?php endif; ?>" id="santri">
						...
						<select name="santri_id" class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
							...
							<?php foreach ($data_santri as $item) : ?>
								<option <?= $data['santri_id'] == $item['id'] ? 'selected' : '' ?> value="<?= $item["id"] ?>"><?= $item["nama"] ?></option>
							<?php endforeach; ?>
						</select>
						<div class="text-red-500 text-xs italic font-semibold">
							<div class="text-red-500 text-xs italic font-semibold">
				...
				<div class="grid grid-cols-4 gap-3">
					...
					<div class="col-span-2">
						...
						<input type="text" placeholder="Masukkan nominal pembayaran" value="<?= set_value('nominal', Helpers::formatRupiah(floatval($data['nominal']))) ?>" name="nominal" id="nominal" class="rupiah bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
						<div class="text-red-500 text-xs italic font-semibold">
							<div class="text-red-500 text-xs italic font-semibold">
				...
				<div class="grid <?php if ($data['jenis_id'] != 7) : ?> hidden <?php endif; ?>" id="keterangan">
