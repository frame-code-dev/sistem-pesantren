...
<div class="p-4 mt-14">
	<section class="p-5 overflow-y-auto">
					...
					<ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
						...
						<li>
							<div class="flex items-center">
								...
								<a href="<?= base_url('dashboard/bulanan') ?>" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Pemasukan</a>
							</div>
						</li>
		...
		<div class="card bg-white p-5 mt-4 border rounded-md w-full relative">
			<form action="<?= base_url('dashboard/bulanan/update/' . $data["id"]) ?>" method="POST" class="w-full mx-auto space-y-4" enctype="multipart/form-data">
				<div class="grid grid-cols-4 gap-3">
					<div class="col-span-2">
						...
						<select disabled id="santri" name="santri" class="select2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
							<option disabled hidden selected value=""> == Pilih Santri == </option>
							<?php foreach ($santri as $row) : ?>
								<option <?= $row["id"] == set_value("santri", $data["santri_id"]) ? "selected" : "" ?> value="<?= $row['id'] ?> "> <?= $row['nama'] ?></option>
							<?php endforeach; ?>
						</select>
					...
					<div class="col-span-2">
						...
						<select id="bulan" name="bulan" class="select2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
							...
							<?php for ($i = 1; $i <= 12; $i++) : ?>
								<option <?= $i == set_value("bulan", $data["bulan"]) ? "selected" : "" ?> <?= $i == set_value("bulan") ? "selected" : "" ?> value="<?= $i ?>"> <?= $bulan[$i - 1] ?></option>
							<?php endfor; ?>
						</select>
						<div class="text-red-500 text-xs italic font-semibold">
					...
					<div class="col-span-2">
						...
						<select id="tahun" name="tahun" class="select2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
							...
							<?php for ($i = $year + 3; $i >= $year - 3; $i--) : ?>
								<option <?= $i == set_value("tahun", $data["tahun"]) ? "selected" : "" ?> value="<?= $i  ?>"><?= $i ?></option>
							<?php endfor; ?>
						</select>
						<div class="text-red-500 text-xs italic font-semibold">
